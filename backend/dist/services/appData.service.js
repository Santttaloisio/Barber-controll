...
const buildMonthReport = (cuts, reference = new Date()) => {
...
const buildYearReport = (cuts, reference = new Date()) => {
    const year = reference.getFullYear();
...
const getAppData = async () => {
    ...
    const [barbersResult, servicesResult, cutsResult, expensesResult] = await Promise.all([
        ...
        supabase_1.supabase.from('cuts').select('*').order('created_at', { ascending: false }),
        supabase_1.supabase.from('expenses').select('*').order('created_at', { ascending: false })
    ]);
